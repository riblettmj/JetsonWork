...
#  Rev. Date:   July 14th, 2017

# PURPOSE:
#  This script automates the configuration and installation of the LLVM 4.0.1
...
mkdir -p llvm-4.0.1.build > /dev/null 2>&1
echo -ne ' - Downloading LLVM core sources\n'
wget http://releases.llvm.org/4.0.1/llvm-4.0.1.src.tar.xz > /dev/null 2>&1
tar xf llvm-4.0.1.src.tar.xz; rm llvm-4.0.1.src.tar.xz > /dev/null 2>&1
cd llvm-4.0.1.src > /dev/null 2>&1
...
wget http://releases.llvm.org/4.0.1/cfe-4.0.1.src.tar.xz > /dev/null 2>&1
mkdir -p clang && tar xf cfe-4.0.1.src.tar.xz -C clang --strip-components=1 > /dev/null 2>&1
rm cfe-4.0.1.src.tar.xz > /dev/null 2>&1

cd ../projects
echo -ne ' - Downloading Compiler-RT sources\n'
wget http://releases.llvm.org/4.0.1/compiler-rt-4.0.1.src.tar.xz > /dev/null 2>&1
mkdir -p compiler-rt && tar xf compiler-rt-4.0.1.src.tar.xz -C compiler-rt --strip-components=1 > /dev/null 2>&1
rm compiler-rt-4.0.1.src.tar.xz > /dev/null 2>&1

echo -ne ' - Downloading libCXX sources\n'
wget http://releases.llvm.org/4.0.1/libcxx-4.0.1.src.tar.xz > /dev/null 2>&1
mkdir -p libcxx && tar xf libcxx-4.0.1.src.tar.xz -C libcxx --strip-components=1 > /dev/null 2>&1
rm libcxx-4.0.1.src.tar.xz > /dev/null 2>&1

echo -ne ' - Downloading libCXX-ABI sources\n'
wget http://releases.llvm.org/4.0.1/libcxxabi-4.0.1.src.tar.xz > /dev/null 2>&1
mkdir -p libcxxabi && tar xf libcxxabi-4.0.1.src.tar.xz -C libcxxabi --strip-components=1 > /dev/null 2>&1
rm libcxxabi-4.0.1.src.tar.xz > /dev/null 2>&1

echo -ne ' - Downloading libunwind sources\n'
wget http://releases.llvm.org/4.0.1/libunwind-4.0.1.src.tar.xz > /dev/null 2>&1
mkdir -p libunwind && tar xf libunwind-4.0.1.src.tar.xz -C libunwind --strip-components=1 > /dev/null 2>&1
rm libunwind-4.0.1.src.tar.xz > /dev/null 2>&1

echo -ne ' - Downloading OpenMP bindings sources\n'
wget http://releases.llvm.org/4.0.1/openmp-4.0.1.src.tar.xz > /dev/null 2>&1
mkdir -p openmp && tar xf openmp-4.0.1.src.tar.xz -C openmp --strip-components=1 > /dev/null 2>&1
rm openmp-4.0.1.src.tar.xz > /dev/null 2>&1

echo -ne ' - Downloading LLVM test suite sources\n'
wget http://releases.llvm.org/4.0.1/test-suite-4.0.1.src.tar.xz > /dev/null 2>&1
mkdir -p test-suite && tar xf test-suite-4.0.1.src.tar.xz -C test-suite --strip-components=1 > /dev/null 2>&1
rm test-suite-4.0.1.src.tar.xz > /dev/null 2>&1


# Configure LLVM build
# ------------------------------------------------------------------------------
echo -ne '\033[32m Configuring LLVM build\033[0m\n'
cd ../../llvm-4.0.1.build > /dev/null 2>&1
cmake -G "Unix Makefiles" -DCMAKE_BUILD_TYPE=Release ../llvm-4.0.1.src/ > /dev/null 2>&1
...
if [ "$STATUS" -eq 0 ]; then
  ...
  STATUS=$?
  if [ "$STATUS" -eq 0 ]; then
    echo -e "\n\033[32m SUCCESS: \033[0m Installed LLVM 4.0.1."
  else
    echo -e "\n\033[31m FAILED: \033[0m Error installing LLVM 4.0.1."
  fi
else
  echo -e "\n\033[31m FAILED: \033[0m Error building LLVM 4.0.1."
fi
